...
public:
    ...
    virtual int  FRead(void* pData, unsigned intuiSize) = 0;
    virtual int  FWrite(const void* pData, unsigned intuiSize) = 0;
};
...
public:
    ...
    virtual void RegisterPacketHandler(void* pfnPacketHandler) = 0;
    ...
    virtual bool               GetNetworkStatistics(void* pDest) = 0;
    ...
    virtual void* GetHTTPDownloadManager(EDownloadModeType iMode) = 0;
    ...
    virtual int  SendTo(void* s, const char* buf, int len, int flags, const struct sockaddr* to, int tolen) = 0;
};
extern CNet* g_pNet;