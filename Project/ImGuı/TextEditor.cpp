...
void TextEditor::HandleKeyboardInputs()
{
    ...
    if (ImGui::IsWindowFocused())
    {
        ...
        if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z))
            ...
            mOverwrite ^= true;
        else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert)) // Eski kýsayol: Ctrl+Insert
            Copy();
        else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_C)) // Yeni kýsayol: Ctrl+C
            Copy();
        else if (!IsReadOnly() && !ctrl && shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert)) // Eski kýsayol: Shift+Insert
            Paste();
        else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_V)) // Yeni kýsayol: Ctrl+V
            Paste();
